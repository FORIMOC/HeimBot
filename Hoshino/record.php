...
if (mysqli_num_rows($sql)>0){
    ...
    $new_freq=$row['freq']+1;
    mysqli_query($conn,"UPDATE Hoshino_users SET freq={$new_freq} WHERE user_id={$data['user_id']}");
}else{
    mysqli_query($conn,"INSERT INTO Hoshino_users (user_id, group_id, freq) VALUES ({$data['user_id']}, {$data['group_id']}, 1)");
}
